...
// Fix: Global declaration for FlutterBridge
declare global {
    interface Window {
        FlutterBridge: any;
        webkit: any;
...
const sendToFlutter = (message: string) => {
    console.log('Attempting to send to Flutter:', message);
    ...
    console.warn('FlutterBridge not found for message:', message);
    return false;
};
...
class AdMobService {
    ...
    async showInterstitial(): Promise<boolean> {
        // Senaryo 3: Flutter WebView
        if (sendToFlutter('show_interstitial')) {
            return true;
        }
    ...
    async reklami_baslat(): Promise<boolean> {
        // Senaryo 3: Flutter WebView
        if (sendToFlutter('show_rewarded')) {
            return true; // We assume the reward will be handled via window.odulVer
        }
