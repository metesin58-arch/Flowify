...
export const SongProductionManager = {
    ...
    calculatePassiveIncome: (discography: ReleasedSong[], managerTier: number = 0): number => {
        ...
        // Manager Bonus: Tier 1 (+20%), Tier 2 (+50%)
        const managerBonus = managerTier === 1 ? 1.2 : managerTier === 2 ? 1.5 : 1;
        income *= managerBonus;
    ...
    // 6. Calculate Passive Listeners Growth per Tick
    calculatePassiveListeners: (discography: ReleasedSong[], careerLevel: number): number => {
        if (!discography || discography.length === 0) {
            return Math.max(1, Math.floor(careerLevel * 0.5)); // Tiny passive baseline
        }
        let listenersGain = 0;
        const now = Date.now();
        discography.forEach(song => {
            const ageInHours = (now - song.releasedAt) / (1000 * 60 * 60);
            const decayFactor = Math.max(0.05, 1 - (ageInHours / 72)); // 3 days to decay
            
            // Base growth derived from song quality, popularity score and decay
            const songGain = (song.quality * (song.popularityScore / 100)) * decayFactor * 0.15;
            listenersGain += songGain;
        });

        // Small boost proportional to career level reputation
        listenersGain += careerLevel * 1.5;

        return Math.max(1, Math.floor(listenersGain));
    }
};
