...
export const RhythmTwisterMode: React.FC<Props> = ({ onExit }) => {
  ...
  useEffect(() => {
    ...
    setVisualState(prev => ({ ...prev, words: initialWords, levelName: "RHYTHM TWISTER" }));
  ...
  return (
    <div className="h-full w-full bg-black flex flex-col relative overflow-hidden font-sans select-none touch-none">
        ...
        {visualState.phase === 'IDLE' ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 relative z-10">
                <div className="relative z-10 text-center max-w-sm px-6 animate-fade-in flex flex-col items-center">
                    ...
                    <div className="w-24 h-24 bg-gradient-to-br from-cyan-400/20 to-black rounded-3xl shadow-[0_0_40px_rgba(34,211,238,0.1)] mb-6 flex items-center justify-center border border-white/10 relative overflow-hidden group">
                        <div className="text-xl font-black text-cyan-400 tracking-tighter">twis<span className="text-white">.</span></div>
                    </div>
                    
                    <span className="text-[9px] font-black tracking-widest text-cyan-400 uppercase mb-1">
                        beceri ve refleks
                    </span>

                    <h1 className="text-4xl font-black text-white tracking-tighter leading-none lowercase mb-3">
                        rhythm twister<span className="text-cyan-400">.</span>
                    </h1>
                    
                    <p className="text-[10px] text-neutral-500 font-medium tracking-tight mb-8 lowercase leading-relaxed">
                        viral tiktok akımı ritim oyunu! ritimleri yakala, ezberle ve zamanlamayı kaybetmeden doğru kelimeleri oku.
                    </p>

                    <button 
                        onClick={startSequence}
                        className="bg-cyan-400 text-black font-black py-3.5 px-10 rounded-2xl hover:scale-102 active:scale-95 transition-all text-[11px] tracking-wide shadow-[0_0_20px_rgba(34,211,238,0.25)] lowercase"
                    >
                        hemen oyna
                    </button>
                </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 relative pb-32 z-10">
                ...
                {visualState.phase === 'FINISHED' && (
                    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/95 backdrop-blur-md p-8 text-center animate-fade-in font-sans">
                        <div className="w-16 h-16 rounded-[2rem] bg-cyan-400/5 flex items-center justify-center mb-6 border border-cyan-400/15 relative">
                            <SpectrumIcon className="w-6 h-6 text-cyan-400 animate-pulse" />
                            <div className="absolute top-0 right-0 w-2 h-2 rounded-full bg-cyan-400 shadow-[0_0_8px_#22d3ee]" />
                        </div>
                        <h2 className="text-4xl font-black text-white tracking-tighter lowercase mb-4">bitti<span className="text-cyan-400">.</span></h2>
                        <div className="bg-[#050505] border border-white/5 p-6 rounded-[2rem] mb-8 w-full max-w-xs shadow-2xl">
                            <p className="text-neutral-400 text-xs font-semibold leading-relaxed lowercase">
                                "{gameOverMsg.toLowerCase()}"
                            </p>
                        </div>
                        ...
                        <div className="flex flex-col gap-3 w-full max-w-xs">
                            <motion.button 
                                ...
                                className="bg-cyan-500 text-black font-extrabold py-4 rounded-3xl text-xs lowercase tracking-tight shadow-[0_0_20px_rgba(6,182,212,0.2)]"
                            >
                                tekrar oyna.
                            </motion.button>
                            <motion.button 
                                whileTap={{ scale: 0.98 }}
                                onClick={onExit}
                                className="bg-transparent border border-white/5 text-neutral-400 font-extrabold py-4 rounded-3xl text-xs lowercase tracking-tight active:text-white active:border-white/20 transition-all font-sans"
                            >
                                çıkış.
                            </motion.button>
                        </div>
        ...
        {/* CONTROLS */}
        {/* Removed redundant start button absolute list overlay */}
